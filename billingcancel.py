...
def billingCancel():
   ...
   # patoolib.extract_archive(Filesdir + "dwh_tr_billing_cancel_ss1.1_20210413_023412.dat.gz",outdir=unzipFilesdir)
    ...
    for a_filename in fileList:
        file_size = os.path.getsize(a_filename)
        print("going to pick it up  ..", a_filename + "Size: " + str(file_size))
        if file_size > 20:
            ...
            dfBillCancelmain = dfBillCancelmain.rename(columns={"ORDER_ID":"ORDERS"})
            print('..File Processed ..' + a_filename)
        else:
            print('..File Skipped ..' + a_filename + "Size : "+ str(file_size))

    dfBillCancelmain.to_csv(Filesdir + 'OutputBefore5.csv')
    print('..Generated..'+Filesdir)
    ...
    dfResult.to_csv(Filesdir + 'OutputCancle5.csv')
